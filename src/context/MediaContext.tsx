...
export const MediaProvider: React.FC<MediaProviderProps> = ({ children }) => {
  ...
  const [activeTab, setActiveTab] = useState<'videos' | 'screenshots' | 'archive'>('videos');
  const [sortOrder, setSortOrder] = useState<'newest' | 'oldest' | 'views' | 'alphabetical'>('newest');
  ...
  const fetchArchivedMedia = async () => {
    ...
    try {
      const [videosRes, screenshotsRes] = await Promise.all([
        axios.get('/api/videos/archived'),
        axios.get('/api/screenshots/archived')
      ]);
  ...
  const takeScreenshot = async (type: 'full' | 'area' | 'tab'): Promise<string | null> => {
    try {
      // Request user permissions
      const displayMediaOptions: DisplayMediaStreamOptions = {
        video: {
  ...
  const archiveMedia = async (id: string, type: 'video' | 'screenshot') => {
    try {
      if (type === 'video') {
        await axios.put(`/api/videos/${id}/archive`);
        fetchVideos();
      } else {
        await axios.put(`/api/screenshots/${id}/archive`);
        fetchScreenshots();
      }
      fetchArchivedMedia();
    } catch (err) {
